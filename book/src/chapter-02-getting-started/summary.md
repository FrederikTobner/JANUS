...
Now we have fully setup the complete OS development environment. Everything you need to write, build, debug, and boot a kernel.
...
[!side]
The hard part of setup is over. Now comes the fun part: making the metal do what you want.
[/!side]
...
>
> 2. **Freestanding vs Hosted** (Experiment): Write a simple C program that > uses `printf`. Try compiling it with `-ffreestanding` vs without. What errors > do you get? Why? Try fixing it by implementing your own `printf` that outputs > to a buffer instead of stdout.
>
...
>    - The C function should use that stack and return a value
>    - Compile both, link them, and verify with `readelf` that the stack is > correctly aligned
>
> 4. **QEMU Machine Exploration** (Research): Run `qemu-system-x86_64 -machine > help`. What machines can QEMU emulate? Research the differences between `pc` > (default) and `q35`. Which is more modern? What advantages does q35 have?
>
...
>    - Build binutils for x86_64-elf target

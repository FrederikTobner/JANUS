...
It's like the difference between reading how to swim and jumping into the deep of the pool. And yes, there are actual books about how to swim and people like Norbert Wiender, a mathmatician, that learned how to swim by reading a book.
Theory teaches you concepts, while practice beats them into your skull with a hammer. 

In application programming, you're multiple abstraction layers away from the hardware. When working on your own operating sytem you're writing directly to memory-mapped I/O ports.

[!side]
When a bug causes mysterious reboots, the lack of a runtime and OS services means you have to figure out what's going wrong. The operaing system would usually send a singal to the process notifying it that a segmentation fault occurred, but here there is no OS to do that for you. Additionally in user space we would be able to just generate a core dump and analyze it later, but here we won't have that luxury either. 
[/!side] 

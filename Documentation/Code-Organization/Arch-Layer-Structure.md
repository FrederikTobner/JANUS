...
## Example: Multiple Contracts and Split Implementation

```
include/example/foo.h
    #include <arch/example/bar.h>
    #include <arch/example/baz.h>

arch/include/arch/example/bar.h
    // declares arch_bar_* or includes impl
arch/include/arch/example/baz.h
    // declares arch_baz_* or includes impl

arch/x86_64/bar.c
arch/x86_64/baz.c
```

## Future: Arch in Libraries

The same structure can be used in `lib/` for utility libraries that need platform-specific code. Simply add an `arch/` directory to the library and follow the same three-tier pattern.

## Summary

- The arch layer is highly flexible: headers and sources can be organized as needed.
- The only rule: Tier 2 declares or includes the arch interface; implementation can be inline or in `.c` files.
- This enables clean, modular, and maintainable platform-specific code for any subsystem or library.
